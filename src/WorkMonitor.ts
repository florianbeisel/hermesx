...
export class WorkMonitor {
  ...
  private checkUserActivity() {
    ...
    if (stateMachine.getState() === WorkState.WORKING) {
      ...
      if (
        idleTime >=
        userConfig.inactivityThresholds.autoBreakSuggestion * 60 * 1000
      ) {
        if (DEBUG.enabled) {
        ...
        if (
          ...
          if (DEBUG.enabled) {
            ...
            console.log(
              ...
              formatDuration(continuousWorkTime)
            );
          }
          notificationManager.scheduleNotification(
            'break-reminder',
      ...
      if (this.expectedBreakReturnTime && now > this.expectedBreakReturnTime) {
        ...
        const lastNotificationTime =
          this.lastNotificationTimes['break-overrun'] || 0;
  ...
  private checkScheduleCompliance() {
    ...
    scheduledEndTime.setHours(
      scheduledEndTime.getHours() + userConfig.schedule.workDuration
    );
    ...
    if (
      ...
      const lastNotificationTime =
        this.lastNotificationTimes[notificationKey] || 0;
      ...
      if (DEBUG.enabled) {
        ...
        console.log(
          'Should notify:',
          now_ms - lastNotificationTime > 5 * 60 * 1000
        );
      }
    ...
    if (
      stateMachine.getState() === WorkState.WORKING &&
  ...
  public onWorkStateChange(newState: WorkState) {
    ...
    switch (newState) {
      ...
      case WorkState.PAUSED:
        ...
        this.expectedBreakReturnTime =
          now + userConfig.schedule.breakDuration * 60 * 1000;
        if (DEBUG.enabled) {
