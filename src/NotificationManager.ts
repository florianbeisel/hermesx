...
export class NotificationManager {
  ...
  private async isInMeetingLegacy(): Promise<boolean> {
    try {
      ...
      const meetingProcesses = processes.filter((proc) =>
        meetingApps.some((app) =>
          proc.toLowerCase().includes(app.toLowerCase())
        )
      );
        ...
        ({ id, notification, timeout }) => {
          ...
          if (
            ...
            timeSinceLastNotification <
              NotificationManager.NOTIFICATION_COOLDOWN
          ) {
          ...
          if (now >= timeout) {
            if (DEBUG.enabled) {
              console.log('Showing notification:', {
                ...
                timeSinceLastNotification: formatDuration(
                  timeSinceLastNotification
                ),
              });
            }
  ...
  public scheduleNotification(
    ...
    const existingNotification = this.notificationQueue.find(
      (n) => n.id === id
    );
