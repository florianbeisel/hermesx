...
import { getCurrentTimestamp, stateMachine, DEBUG, performAction, formatDuration, userConfig, notificationManager, getCurrentTime } from './main';
...
export class WorkMonitor {
        ...
        setTimeout(() => {
          if (stateMachine.getState() === WorkState.WORKING) {
            ...
            notification2.on('action', async () => {
              const pauseAction = STATE_ACTIONS[WorkState.WORKING].find(action => action.nextState === WorkState.PAUSED);
              if (pauseAction) {
  ...
  private checkUserActivity() {
    ...
    if (DEBUG.enabled) {
      ...
      if (this.expectedBreakReturnTime) {
        console.log('Expected break return:', new Date(this.expectedBreakReturnTime).toLocaleTimeString());
        console.log('Time until return:', formatDuration(this.expectedBreakReturnTime - now));
      }
    }
    ...
    if (stateMachine.getState() === WorkState.WORKING) {
      ...
      if (this.continuousWorkStartTime) {
        ...
        if (continuousWorkTime >= userConfig.inactivityThresholds.shortBreakReminder * 60 * 1000) {
          if (DEBUG.enabled) {
    ...
    if (idleTime < 60 * 1000) { // Less than 1 minute idle
  ...
  private checkScheduleCompliance() {
    ...
    if (DEBUG.enabled) {
      ...
      console.log('Current state:', stateMachine.getState());
      console.log('Minutes late:', Math.floor((now.getTime() - scheduledStartTime.getTime()) / (60 * 1000)));
    }

    // Check start time compliance
    if (stateMachine.getState() === WorkState.NOT_WORKING && !stateMachine.isFinishedForToday() && now > scheduledStartTime) {
      const minutesLate = Math.floor((now.getTime() - scheduledStartTime.getTime()) / (60 * 1000));
      ...
      if (DEBUG.enabled) {
        ...
        console.log('Last notification:', new Date(lastNotificationTime).toLocaleTimeString());
    ...
    if (stateMachine.getState() === WorkState.WORKING && now > scheduledEndTime) {
      const minutesOver = Math.floor((now.getTime() - scheduledEndTime.getTime()) / (60 * 1000));
    ...
    switch (newState) {
      ...
      case WorkState.PAUSED:
        ...
        if (DEBUG.enabled) {
          ...
          console.log('Expected return at:', new Date(this.expectedBreakReturnTime).toLocaleTimeString());
        }
        break;
